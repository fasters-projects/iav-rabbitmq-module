...
export declare class RabbitSetupService<Q extends string, E extends string, R extends string> implements OnModuleInit {
    ...
    createDeadLetterQueue(queueName: string, deadLetterExchange: string, deadLetterRoutingkey?: string): Promise<void>;
    createDelayQueue(queueName: string, deadLetterExchange: string, delayTime?: number, deadLetterRoutingkey?: string, maxRetries?: number): Promise<void>;
    ...
    setupQueue({ delayTime, exchange, maxRetries, queue, routingKeys, extraDlqQueue, }: QueueBindConfig<Q, E, R>): Promise<void>;
    setupExchangesWithRetryDlqAndDelay(exchange: string): Promise<void>;
    setupQueuesWithRetryAndDelay({ delayTime, exchange, queue, routingKeys, maxRetries, }: QueueBindConfig<Q, E, R>): Promise<void>;
